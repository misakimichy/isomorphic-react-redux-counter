...
describe("Counter component", () => {
    ...
    it('third button should not call onIncrement if the counter is even', () => {
        const { buttons, actions } = setup(22)
        buttons.at(2).simulate('click')
        expect(actions.onIncrement).not.toBeCalled()
    })

    it('third button should  all onIncrement if the counter is odd', () => {
        const { buttons, actions } = setup(23)
        buttons.at(2).simulate('click')
        expect(actions.onIncrement).toBeCalled()
    })
    ...
    it('Forth button should call onIncrement in a second', done => {
        const { buttons, actions } = setup()
